...
vector<vaccination> vaccination::findBook(string vaccine_date, string pet_name, string user_name, int vaccine_id, int pet_id, int user_id, bool ascending)
{

	string query = "SELECT * FROM vaccination v JOIN pet p ON v.pet_id = p.pet_id JOIN user u ON p.user_id = u.user_id WHERE 1=1 AND u.user_phone = " + to_string(user_id);
